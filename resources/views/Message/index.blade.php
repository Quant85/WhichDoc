                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="form-group">
                                            <label for=""></label>
                                            <textarea class="form-control" name="" id="" rows="2" disabled>{{$message->testo_messaggio}}</textarea>
                                        </div>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#destroy-{{ $message->id }}"><i class="fas fa-broom"></i></button>
                                    </td>
                                    {{-- Start Add Modal -  --}}
                                    <div class="modal fade" id="destroy-{{ $message->id }}" tabindex="-1" role="dialog" aria-labelledby="message-destroy-{{ $message->id }}" aria-hidden="true">
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <h5 class="modal-title" id="#destroy-{{ $message->id }} title">Elimina Messaggio di {{$message->nome_paziente}}<i class="fas fa-broom"></i></h5>
                                            ...
                                            <form action="{{ route('medico.messaggi.destroy', $message->id)}}" method="post">
                                                @csrf
                                                @method('DELETE')
                                                <button class="btn btn-danger" type="submit"><i class="fas fa-broom"></i></button>
                                            </form>
                                            </div>
                                        </div>
                                        </div>
